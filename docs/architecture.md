...
GPR is a raw image compression codec built on GoPro's open-source VC5/CineForm wavelet codec. The v2.0 extensions add noise-aware compression and adaptive entropy coding, supporting multiple bit depths: 12-bit (GoPro Hero5/6), 14-bit (GoPro Hero7-12, 45MP DSLRs), and 16-bit (100MP medium format sensors).
...
The original VC5 codec uses a fixed Huffman codebook (Table 17) designed for GoPro's typical coefficient distribution. It works well for GoPro but can't adapt to different cameras, ISOs, or noise levels. For higher-resolution 14-bit and 16-bit sensors, the coefficient statistics differ enough that VLC wastes 20-40% of bits.
...
| 45MP 14-bit | Encode (ANS+DN) | 1.0s | 45 MP/s |
| 45MP 14-bit | Decode | 0.5s | 90 MP/s |
| 100MP 16-bit | Encode (ANS+DN) | 2.5s | 40 MP/s |
| 100MP 16-bit | Decode | 1.2s | 83 MP/s |
...
| Nikon Z8 (45MP) | 91 MB | 15.5 MB | **15.0 MB** | **6.1x** |
| Hasselblad X2D (100MP) | 204 MB | — | **36.1 MB** | **5.7x** |

GoPro benefits most from Joint RLV (39% better than VLC). Per-band auto-selection ensures ANS is never larger than VLC — the Z8 at low ISO is 3% smaller because the encoder picks VLC for bands where it wins. At ISO 1600+, ANS+DN consistently beats VLC by 20-29%.

### Where Time Goes (45MP 14-bit Encode Profile)
...
### Where Time Goes (45MP 14-bit Decode Profile)
...
|--------|-------------|----------|-----------------|
| 45MP 14-bit DSLR | 153 | 0 | 7.0x |
| 100MP 16-bit MF | 4 | 0 | 5.0x |
| GoPro (Hero5-10) | 6 | 0 | 4.6x |

Previous scan (before Joint RLV): 6,713 14-bit DSLR files with 2 outliers at 43.5 dB PSNR (high-ISO images, working as designed).
...
1. **No .fff/.3FR support**: Some medium format raw files require manual conversion to DNG via rawpy or Adobe DNG Converter
...
| File | Lines | Purpose |
|------|-------|---------|
| `source/lib/vc5_encoder/encoder.c` | 3060 | Encoder pipeline: phases 0.5 → 1 → 1.8 → 2, embedded mode |
| `source/lib/vc5_decoder/decoder.c` | 2530 | Decoder pipeline: ANS dispatch (modes 1-4), band decode |
| `source/lib/vc5_common/ans_joint.c` | 740 | Joint RLV ANS: encode/decode, x4 interleaved, arena allocator |
| `source/lib/vc5_common/ans_joint.h` | 100 | Joint RLV types: JANS_TABLE with rcp_freq, decode entries |
| `source/lib/vc5_common/ans.c` | 410 | Legacy separate run+mag ANS (decoder backward compat) |
| `source/lib/vc5_common/noise_model.c` | 530 | Noise remove/restore, FPN model, PRNG |
| `source/lib/vc5_encoder/denoise.c` | 570 | BayesShrink, MAD estimation, adaptive quant |
| `source/lib/vc5_decoder/dequantize.c` | 190 | Inverse quantization, negative quant sentinel, NEON |
...
| `source/app/gpr_tools/main.cpp` | 230 | CLI tool: -A, -D, -E, -R, -F flags |
| `source/app/compare_quality/compare_quality.c` | 240 | PSNR, SSIM, noise preservation metrics |
| `source/app/ans_test/ans_test.c` | 210 | ANS round-trip unit test |
| `source/app/fuzz_ans/fuzz_ans.c` | 100 | libFuzzer target for ANS decode |
